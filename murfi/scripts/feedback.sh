#! /bin/bash
# Mindfulness Neurofeedback Pipeline (Young Lab, Pitt)
# Based on rt-BPD pipeline by Clemens Bauer, adapted from Paul Bloom (2022)
...
# Resolve script location (works both when sourced and executed directly)
...
# Set initial paths
subj_dir=${SCRIPT_PATH}/../subjects/$subj
absolute_path="$(cd "${SCRIPT_PATH}/.." && pwd)"
subj_dir_absolute="$(cd "${SCRIPT_PATH}/../subjects" 2>/dev/null && pwd)/$subj"
MASKS_DIR=${SCRIPT_PATH}/masks
fsl_scripts=${SCRIPT_PATH}/fsl_scripts

export MURFI_SUBJECTS_DIR="${absolute_path}/subjects/"
export MURFI_SUBJECT_NAME=$subj

MURFI_CONTAINER="/opt/murfi/apptainer-images/murfi.sif"
...
template_dmn="${MASKS_DIR}/DMNax_brainmaskero2.nii"
template_cen="${MASKS_DIR}/CENa_brainmaskero2.nii"
...
then
    ...
    echo "+ Single-machine setup: PsychoPy runs on localhost (127.0.0.1)"
    echo "make sure Wi-Fi is off"
    echo "make sure you are Wired Connected to rt-fMRI"
    echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    # Check firewall allows scanner network to reach port 50000
    # nftables is the actual enforcer; ufw status alone is unreliable (Docker adds chains)
    if command -v nft &>/dev/null; then
        nft_input=$(sudo -n nft list chain ip filter ufw-user-input 2>/dev/null || true)
        if [ -n "$nft_input" ]; then
            if ! echo "$nft_input" | grep -q "dport 50000"; then
                echo ""
                echo "  *** WARNING: Firewall is BLOCKING port 50000 ***"
                echo "  The scanner will NOT be able to send data to this machine."
                echo "  Fix: sudo ufw allow from 192.168.2.0/24 to any port 50000 proto tcp"
                echo ""
            elif echo "$nft_input" | grep "dport 50000" | grep -q "saddr 192.168.2.1 "; then
                echo ""
                echo "  *** WARNING: Port 50000 allowed only from 192.168.2.1 ***"
                echo "  The MARS (reconstruction computer) has a different IP."
                echo "  Fix: sudo ufw allow from 192.168.2.0/24 to any port 50000 proto tcp"
                echo ""
            else
                echo "+ Firewall: port 50000 open for scanner network"
            fi
        fi
    fi
fi  
...
then
    ...
    run_murfi_container "$subj_dir_absolute/xml/2vol.xml"
fi
...
clear
    ...
    run_murfi_container "$subj_dir_absolute/xml/rtdmn.xml"
fi
...
clear
    ...
    run_murfi_container "$subj_dir_absolute/xml/rest.xml"
fi
...
then
    ...
    touch ${correlfile}
    template_networks="${MASKS_DIR}/template_networks.nii"
    ...
    then
        ...
        flirt -in ${MASKS_DIR}/MNI152_T1_2mm_brain -ref ${examplefunc} -out ${mni2example_func} -omat ${mni2native_mat}
        ...
        flirt -in ${examplefunc} -ref ${MASKS_DIR}/MNI152_T1_2mm_brain -out ${example_func2mni} -omat ${example_func2mni_mat} -dof 12 -cost corratio
        ...
        flirt -in ${MASKS_DIR}/MNI152_T1_2mm_brain -ref ${examplefunc} -out ${mni2example_func} -init ${mni2example_func_mat} -applyxfm
    ...
    if [ $ica_version == 'multi_run' ]; then
        ...
        flirt -in ${dmn_thresh} -ref ${MASKS_DIR}/MNI152_T1_2mm_brain -out ${subj_dir}/mask/mni/dmn_mni.nii -init ${example_func2mni_mat} -applyxfm -interp nearestneighbour -datatype short
        flirt -in ${cen_thresh} -ref ${MASKS_DIR}/MNI152_T1_2mm_brain -out ${subj_dir}/mask/mni/cen_mni.nii -init ${example_func2mni_mat} -applyxfm -interp nearestneighbour -datatype short
    fi
...
then
    clear
    mni_template=${MASKS_DIR}/MNI152_T1_2mm_LPS_brain.nii
    dmn_mni=${MASKS_DIR}/DMNax_brainmaskero2_lps.nii
    cen_mni=${MASKS_DIR}/CENa_brainmaskero2_lps.nii
